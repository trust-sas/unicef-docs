...
const AddDocumentPage = () => {
  ...
  const [imageCouverture, setImageCouverture] = useState<{name: string, url: string, size: number} | null>(null);
  ...
  const analyzeContentWithAI = async (text: string) => {
    try {
      const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        method: "POST",
        headers: {
          "Authorization": "Bearer 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",
        ...
        try {
          ...
          if (extractedText) {
            ...
            if (aiAnalysis) {
              ...
              setMessage({ type: 'success', text: 'Fichier uploadé et informations extraites automatiquement!' });
            }
          }
        } catch (extractError) {
          console.error('Erreur lors de l\'extraction:', extractError);
          setMessage({ type: 'warning', text: 'Fichier uploadé mais extraction automatique échouée. Veuillez remplir manuellement.' });
        } finally {
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="flex items-center gap-3">
            <div>
              ...
              <p className="text-blue-100 mt-1">Partagez vos ressources avec la communauté UNICEF</p>
            </div>
          </div>
        </div>
      </div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                {uploading || extractingData ? (
                  ...
                  <div className="flex items-center justify-center gap-4">
                    <div className="w-8 h-10 bg-red-500 rounded text-white text-xs flex items-center justify-center font-bold">PDF</div>
